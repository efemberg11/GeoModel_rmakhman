...
// use the local version of the G4GammaGeneralProcess till Geant4-11.0
  ...
  : G4VPhysicsConstructor(name), verbose(ver), fWDCKRegionName(""), fWDCKLowEnergyThreshold(0.0)
{
  ...
  // activate gamma-general process in order to add all (even non EM) proesses
...
StandardEmWithWoodcock::~StandardEmWithWoodcock()
{}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

void StandardEmWithWoodcock::ConstructParticle()
{
  // gamma
  G4Gamma::Gamma();
  // leptons
...
//
// NOTE: M. Novak
// Using old style EM process construction that good for G4.10.6p03 as well
void StandardEmWithWoodcock::ConstructProcess()
{
  if(verbose > 1) {
  ...
  for(const auto& particleName : partList.PartNames()) {
    ...
    if (particleName == "gamma") {
      //
      // Create a Woodcock process, add all EM Gamma processes (in the same way as in
      // case of the G4GammaGeneralProcess). Note that further processes (extra and
      // non-EM like gamma-nuclear) might be added automatically in later physics
      // constructors just as in case of the Gamma-Generel process. This is done
      // automatically when the general-process is activated, e.g. by invoking
      // `G4EmParameters::Instance()->SetGeneralProcessActive(true)` or through the
      // `/process/em/UseGeneralProcess true` UI command.
      //
      //
      // Woodcock tracking will be required inside the `theWDCKregionName`
      ...
      theWDCKProcess->AddEmProcess(pee);
      theWDCKProcess->AddEmProcess(new G4ComptonScattering);
      theWDCKProcess->AddEmProcess(new G4GammaConversion);
      theWDCKProcess->AddEmProcess(new G4RayleighScattering);
      //
      G4LossTableManager::Instance()->SetGammaGeneralProcess(theWDCKProcess);
      ph->RegisterProcess(theWDCKProcess, particle);
