...
bool MagField::AtlasFieldSvc::initializeMap(AtlasFieldSvcTLS &tls)
{
    ...
    if ( strstr(mapFile.c_str(), ".root") != 0 )
    {
	    ...
	    std::cout<<"You may copy the map from the location:" << std::endl << std::endl;
	    std::cout<< "https://geomodel.web.cern.ch/atlas-magnetic-field/bmagatlas_09_fullAsym20400.data" << std::endl <<  std::endl;
	    std::cout << "to " << std::endl << std::endl << std::string(dirname((char *) resolvedMapFile.c_str())) << std::endl;
            exit (-1);
            return false;
        }
