...
: G4VUserActionInitialization(), fIsPerformance(isperformance),fCustomUserActions(customuseractions),
  fSpecialScoringRegionName("") {
      ...
      std::cout << "Hello from FSLActionInitialization " << std::endl;
  }
...
void FSLActionInitialization::Build() const {
  ...
  std::cout << "Hello from FSLActionInitialization Build " << std::endl;
#if !USE_PYTHIA
...
#endif
      std::cout << "Hello Loading in event generator from plugin" << std::endl;
