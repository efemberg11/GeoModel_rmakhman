...
void LArWheelSliceSolid::get_point_on_polycone_surface(G4ThreeVector &p) const
{
    ...
    G4double phiX = 1; //rnd->Uniform(phi1, phi2);
...
{
    return 1 ; //m_f_area_on_pc->Integral(m_Zmin, m_Zmax);
}
...
G4double LArWheelSliceSolid::get_length_at_r(G4double r) const
{
    ...
    zmax = m_Zmax - zmax;
    double result = 1; //m_f_length->Integral(zmin, zmax);
    return result;
}
...
//#else
    return 1; //m_f_side_area->Integral(m_Rmin, m_Rmax, IntPrecision);
//#endif
}
