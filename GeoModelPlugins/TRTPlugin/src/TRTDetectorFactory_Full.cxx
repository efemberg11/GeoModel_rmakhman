...
{
  double ptot2 = a.squaredNorm()*b.squaredNorm();
...
/////////////////////////////////// Constructor //////////////////////////////////
//
TRTDetectorFactory_Full::TRTDetectorFactory_Full(bool useOldActiveGasMixture
...
void TRTDetectorFactory_Full::create(GeoPhysVol *world)
{
  ...
  std::cout <<  " Getting primary numbers from the Detector Description Database " << std::endl;
  TRT_DetDescrDB_ParameterInterface * parameterInterface = new TRT_DetDescrDB_ParameterInterface();
  m_data = parameterInterface;

  //---------------------- Get material manager ------------------------//
  m_materialManager = GeoXmlMatManager::getManager();
  ...
  if (!m_strawsvcavailable) std::cout << "The folder of /TRT/Cond/StatusHT is NOT available, WHOLE TRT RUNNING XENON" << std::endl;
  if (!m_doArgon  )	std::cout << "Tool setup will force to NOT to use ARGON. Ignore this warning if you are running RECONSTRUCTION or DIGI, but cross-check if you are running SIMULATION" << std::endl;
  if (!m_doKrypton)	std::cout <<  "Tool setup will force to NOT to use KRYPTON. Ignore this warning if you are running RECONSTRUCTION or DIGI, but cross-check if you are running SIMULATION" << std::endl;
...
//  std::cout <<  "In TRT Detector Factory (For DC2 and later geometries)" << std::endl;
//  std::cout <<  " " << version.fullDescription() << std::endl;
...
//  const int AlignmentLevelTop       = 3; // Level 1
  ...
  TRT_Numerology numerology;
  
  for (unsigned int m=0;m<m_data->nBarrelRings;m++) {
    numerology.setNBarrelLayers(m, m_data->barrelNumberOfStrawLayersInModule[m]);
  }

  numerology.setNBarrelRings(m_data->nBarrelRings);
  //Note: This next line is now consistent with TRT_TestBeamDetDescr.
  numerology.setNBarrelPhi(m_data->nBarrelModulesUsed);
  ...
  numerology.setNEndcapWheels(nEndcapWheels);
  numerology.setNEndcapPhi(m_data->nEndcapPhi);

  for (unsigned int w=0;w<numerology.getNEndcapWheels();w++) {
    ...
    numerology.setNEndcapLayers(w, nlayers ) ;
  }
  ...
  if (barrelPresent) {
    ...
    std::cout <<  "Virtual TRT Barrel volume defined by RMin = "<<m_data->virtualBarrelInnerRadius 
	      <<", Rmax = "<<m_data->virtualBarrelOuterRadius<<" Zmax = "<<m_data->virtualBarrelVolumeLength << std::endl;
...
//    m_detectorManager->addAlignableTransform(AlignmentLevelTop, id, transform, pEndCapABPlus); // global if other selected
  ...
  if (endcapABMinusPresent) {
    ...
    GeoAlignableTransform * transform =
      new GeoAlignableTransform(trtTransform * m_data->partTransform(endcapC_WheelAB_Label) * GeoTrf::RotateY3D(180*SYSTEM_OF_UNITS::deg));
...
//    m_detectorManager->addTreeTop(pEndCapCPlus);
  ...
  if (endcapCMinusPresent) {
    ...
    GeoAlignableTransform * transform =
      new GeoAlignableTransform(trtTransform * m_data->partTransform(endcapC_WheelC_Label) * GeoTrf::RotateY3D(180*SYSTEM_OF_UNITS::deg));
...
//    std::vector<InDetDD::TRT_BarrelDescriptor *> bDescriptor;
    ...
    for (size_t iABC=0;iABC<m_data->nBarrelRings;iABC++) {
      ...
      if ( shellCorner1.y() <= 0 ) { std::cout <<  "shellCorner1 is <= 0 (" << shellCorner1 << ")" << std::endl; }
      if ( shellCorner2.y() <= 0 ) { std::cout <<  "shellCorner2 is <= 0 (" << shellCorner2 << ")" << std::endl; }
      if ( shellCorner3.y() <= 0 ) { std::cout <<  "shellCorner3 is <= 0 (" << shellCorner3 << ")" << std::endl; }
      if ( shellCorner4.y() <= 0 ) { std::cout <<  "shellCorner4 is <= 0 (" << shellCorner4 << ")" << std::endl; }
	...
	for (size_t iLayer = 0; iLayer<m_data->barrelNumberOfLayersWithLargeDeadRegion; iLayer++) {
      ...
      double zrotang = std::atan2(global12[1],global12[0]) - std::atan2(local12[1],local12[0]);
...
//	m_detectorManager->setBarrelTransformField(iABC,tx2.clone());
	...
	//AALONSO
      ...
      double oldx=-999*SYSTEM_OF_UNITS::cm, oldz=-999*SYSTEM_OF_UNITS::cm;
      unsigned int c=0;
      size_t iLayer=0;
	...
	if (sqrt((x-oldx)*(x-oldx)+ (z-oldz)*(z-oldz))> 5*SYSTEM_OF_UNITS::cm) {
	  iLayer++;
...
//	bDescriptor.back()->addStraw(z,x);
	...
	double delta = iMod*360*SYSTEM_OF_UNITS::deg/m_data->nBarrelModules;
...
//  ActiveGasMixture agm = DecideGasMixture(strawStatusHT);
  ...
  switch (agm)
    {
    case GM_ARGON:
//      std::cout <<  "Marking Argon straws from /TRT/Cond/StatusHT:\t"
//		<< idHelper->print_to_string(TRT_Identifier) << std::endl; 
      pShell->add(pRadAR);
      break;
    case GM_KRYPTON:
//      std::cout <<  "Marking Krypton straws from /TRT/Cond/StatusHT:\t"
//		<< idHelper->print_to_string(TRT_Identifier) << std::endl;
      pShell->add(pRadKR);
      break;
    case GM_XENON:
//      std::cout <<  "Marking Xenon straws from /TRT/Cond/StatusHT:\t"
//		<< idHelper->print_to_string(TRT_Identifier)  << std::endl;
      pShell->add(pRad);
      break;
    default:
      std::cerr << "Unexpected gas mixture: " << agm << std::endl;
	...
	unsigned int nStrawLayers = numerology.getNBarrelLayers(iABC);
...
//	  if (iABC>0) jStrawLayer += numerology.getNBarrelLayers(0);
//	  if (iABC>1) jStrawLayer += numerology.getNBarrelLayers(1);
...
//	  for (unsigned int s=0;s<numerology.getNBarrelLayers(iABC); s++) {
...
//    }
  ...
  unsigned int indexUpperBound = firstIndexOfA + numerology.getNEndcapWheels();
  ...
  if (endcapABPlusPresent || endcapABMinusPresent) {
    ...
    double deltaPhiForStrawsA = 360.*SYSTEM_OF_UNITS::deg/m_data->endcapNumberOfStrawsInStrawLayer_AWheels;
...
//		      double startPhi = phiPlane + iiPhi * pDescriptor->strawPitch() * pDescriptor->nStraws();
		      ...
		      //   phi -> pi - phi
		      if (iiSide) {
//			startPhi = SYSTEM_OF_UNITS::pi - (startPhi + pDescriptor->strawPitch() * (pDescriptor->nStraws() - 1));
		      }
		      
		      // Make sure its between -pi and pi.
//		      if (startPhi <= -SYSTEM_OF_UNITS::pi) startPhi += 2*SYSTEM_OF_UNITS::pi;
//		      if (startPhi > SYSTEM_OF_UNITS::pi) startPhi -= 2*SYSTEM_OF_UNITS::pi;
...
//    pStrawPlaneB->ref();
    ...
    // This is the straw pitch.
    double deltaPhiForStrawsB = 360.*SYSTEM_OF_UNITS::deg/m_data->endcapNumberOfStrawsInStrawLayer_BWheels;
...
//	    m_detectorManager->addAlignableTransform(AlignmentLevelModule, idModule, xfWheel, pWheelB); 	    
  ...
  if (endcapCPlusPresent || endcapCMinusPresent) {
    ...
    // This is the straw pitch.
    double deltaPhiForStrawsC = 360.*SYSTEM_OF_UNITS::deg/m_data->endcapNumberOfStrawsInStrawLayer_CWheels;
...
//	    m_detectorManager->addAlignableTransform(AlignmentLevelModule, idModule, xfWheel, pWheelC); 	    
  ...
  // 	  for (iiPlane=0; iiPlane<numerology.getNEndcapLayers(iiWheel); iiPlane++)
  // 	    {
...
const GeoShape * TRTDetectorFactory_Full::makeModule ( double length, GeoTrf::Vector2D corner1, GeoTrf::Vector2D corner2,
  ...
  int sign = ( openingAngleOfFirstCorner < 90*SYSTEM_OF_UNITS::deg ? 1 : -1);
  ...
  double epsilon = 1*SYSTEM_OF_UNITS::micrometer; // needed to ensure perfect overlaps.
  ...
  double modRot = std::atan2(-delta12[1],-delta12[0]) - std::atan2(1,0);
  ...
  modulePosition = GeoTrf::TranslateY3D(center.y())*GeoTrf::TranslateX3D(center.x())*GeoTrf::RotateZ3D(90.0*SYSTEM_OF_UNITS::deg)*GeoTrf::RotateX3D(90.0*SYSTEM_OF_UNITS::deg)*GeoTrf::RotateY3D(modRot);
...
GeoFullPhysVol * TRTDetectorFactory_Full::makeStrawPlane(size_t w, ActiveGasMixture gasMixture) {
  ...
  Sin sin;
  Cos cos;
  TRANSFUNCTION tx =  Pow(Tx,pos*cos(dphi*i))*Pow(Ty,pos*sin(dphi*i))*Pow(Rz,dphi*i)*GeoTrf::RotateY3D(-90*SYSTEM_OF_UNITS::deg);
