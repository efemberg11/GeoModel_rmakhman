...
TopLevelPlacements::TopLevelPlacements(GeoInpRecordset_ptr topLevelTable)
...
void
TopLevelPlacements::fillPlacements(GeoInpRecordset_ptr topLevelTable) {
  ...
  for (int i = 0; i < numParts; i++) {
    const GeoInpRecord& record = (*topLevelTable)[i];
...
GeoTrf::Transform3D
TopLevelPlacements::partTransform(const GeoInpRecord& record) const {
  double posX = record.getDouble("POSX") * SYSTEM_OF_UNITS::mm;
  double posY = record.getDouble("POSY") * SYSTEM_OF_UNITS::mm;
  double posZ = record.getDouble("POSZ") * SYSTEM_OF_UNITS::mm;
  double rotX = record.getDouble("ROTX") * SYSTEM_OF_UNITS::degree;
  double rotY = record.getDouble("ROTY") * SYSTEM_OF_UNITS::degree;
  double rotZ = record.getDouble("ROTZ") * SYSTEM_OF_UNITS::degree;
