...
void SoCooperativeSelection::handleEvent(SoHandleEventAction * action)
{
  ...
  if (SO_MOUSE_PRESS_EVENT(event, BUTTON1)) {
	/* for debug
	std::cout << "SO_MOUSE_PRESS_EVENT : BUTTON1" << std::endl;
	std::cout << "mouse position: " << event->getPosition().toString().getString()
			<< ", mouse normalized position: " << event->getNormalizedPosition( action->getViewportRegion() ).toString().getString()
			<< std::endl;
    */
    if (this->mouseDownPickPath) {
      this->mouseDownPickPath->unref();
    ...
    if (pp) {
      ...
      //std::cout << "pickedPoint - getPoint: " << pp->getPoint().toString().getString() << std::endl; //for debug
      ...
      if (selectionpath&&lastsoselectionfrompath==this) {
    	  //We are the selection node farthest down the path => handle the event.
	...
	//std::cout << "SO_MOUSE_RELEASE_EVENT : BUTTON1" << std::endl; // for debug
