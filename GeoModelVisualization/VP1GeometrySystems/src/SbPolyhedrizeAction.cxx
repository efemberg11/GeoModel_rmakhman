...
void SbPolyhedrizeAction::handleTorus(const GeoTorus *torus)
{
  m_polyhedron = new SbPolyhedronTorus (torus->getRMin(),
				      torus->getRMax(),
				      torus->getRTor(),
				      torus->getSPhi(),
				      torus->getDPhi());
...
{
	std::cout << "SbPolyhedrizeAction::handleTessellatedSolid" << std::endl;
  ...
  for(size_t facetIndInSolid = 0; facetIndInSolid<tessellated->getNumberOfFacets(); ++facetIndInSolid) {
	...
	found = (vertexToCheck-vertex).norm() < vertTolerance;
	if(!found) j++;
      }
