...
LArWheelCalculator::LArWheelCalculator(LArG4::LArWheelCalculator_t a_wheelType
  ...
  m_fanCalcImpl(0)
{
  ...
  std::cout << "LArWheelCalculator constructor at " << this
      << " (type " << LArWheelCalculatorTypeString(m_type)
      << "):" << std::endl;

  // Access source of detector parameters.
  std::cout << "initializing data members from DB..." << std::endl;

  m_zWheelRefPoint = parameters.m_zWheelRefPoint;
  m_dMechFocaltoWRP = parameters.m_dMechFocaltoWRP;
  m_dElecFocaltoWRP = parameters.m_dElecFocaltoWRP;
  m_HalfGapBetweenWheels = parameters.m_HalfGapBetweenWheels;
  m_rOuterCutoff = parameters.m_rOuterCutoff;
  m_zShift = parameters.m_zShift;
  m_eta_hi = parameters.m_eta_hi;
  m_eta_mid = parameters.m_eta_mid;
  m_eta_low = parameters.m_eta_low;
  m_ActiveLength = parameters.m_ActiveLength;
  m_StraightStartSection = parameters.m_StraightStartSection;
  m_dWRPtoFrontFace = parameters.m_dWRPtoFrontFace;

  m_WheelThickness = m_ActiveLength + 2.*m_StraightStartSection;
  m_HalfWheelThickness = m_WheelThickness * 0.5;

  std::string pr_opt_value = parameters.m_pr_opt_value;
  std::string sagging_opt_value = parameters.m_sagging_opt_value;
  ...
  std::cout << "... got these values:" << std::endl
      << "m_zWheelRefPoint       : " << m_zWheelRefPoint / SYSTEM_OF_UNITS::cm << " [cm]" << std::endl
      << "m_dMechFocaltoWRP      : " << m_dMechFocaltoWRP / SYSTEM_OF_UNITS::cm << " [cm]" << std::endl
      << "m_dElecFocaltoWRP      : " << m_dElecFocaltoWRP / SYSTEM_OF_UNITS::cm << " [cm]" << std::endl
      << "m_HalfGapBetweenWheels : " << m_HalfGapBetweenWheels / SYSTEM_OF_UNITS::cm << " [cm]" << std::endl
      << "m_rOuterCutoff         : " << m_rOuterCutoff / SYSTEM_OF_UNITS::cm << " [cm]" << std::endl
      << "m_zWheelFrontFace      : " << m_zWheelFrontFace / SYSTEM_OF_UNITS::cm << " [cm]" << std::endl
      << "m_zWheelBackFace       : " << m_zWheelBackFace / SYSTEM_OF_UNITS::cm << " [cm]" << std::endl
      << "m_zShift               : " << m_zShift / SYSTEM_OF_UNITS::cm << " [cm]" << std::endl
      << "Phi rotation           : " << (m_phiRotation? "true": "false") << std::endl
      << "eta wheels limits      : " << m_eta_low << ", " << m_eta_mid << ", " << m_eta_hi
      << std::endl;
  std::cout << "hardcoded constants: " << std::endl
      << "m_WheelThickness       : " << m_WheelThickness / SYSTEM_OF_UNITS::cm << " [cm]" << std::endl
      << "m_dWRPtoFrontFace      : " << m_dWRPtoFrontFace / SYSTEM_OF_UNITS::cm << " [cm]"
      << std::endl;
  ...
  std::cout << "SAGGING value = " << sagging_opt_value << std::endl;
  ...
  if (m_SaggingOn) {
    std::cout << "Creating DistanceCalculatorSaggingOn = "  << this
        << ',' << m_distanceCalcImpl << std::endl;
  } else {
    std::cout << "Creating DistanceCalculatorSaggingOff = " << this
        << ',' << m_distanceCalcImpl << std::endl;
  }
  ...
  std::cout << "Loading SlantAngle parameters ...";
  std::string slant_params = (m_isInner ? parameters.m_slant_params_inner : parameters.m_slant_params_outer);

  std::cout << (m_isInner?" InnerWheel ":" OuterWheel ") << slant_params << std::endl;

  if(slant_params != "" && slant_params != "default"){
    double a, b, c, d, e;
    if(sscanf(slant_params.c_str(), "%80le %80le %80le %80le %80le", &a, &b, &c, &d, &e) != 5){
      std::cerr << "LArWheelCalculator: ERROR: wrong value(s) "
...
//  fill_sincos_parameterization(); // initialize sin&cos parameterization
  ...
  std::cout << "All params initialized. Print some internal variables" << std::endl;
  ...
  std::cout << "Data members:" << std::endl
      ...
      << "Slant parameters          : ";
  for(int i = 0; i < 5; i ++) std::cout << " " << m_slant_parametrization[i];
  std::cout << std::endl;
...
void LArWheelCalculator::inner_wheel_init(const LArWheelCalculatorParameters& parameters)
{
  for(int i = 0; i < 5; ++ i) {
  ...
  m_NumberOfFans=parameters.m_NumberOfFans1;
  m_NumberOfWaves=parameters.m_NumberOfWaves1;
...
void LArWheelCalculator::outer_wheel_init(const LArWheelCalculatorParameters& parameters)
{
  for(int i = 0; i < 5; ++ i) {
  ...
  m_NumberOfFans=parameters.m_NumberOfFans2;
  m_NumberOfWaves=parameters.m_NumberOfWaves2;  
