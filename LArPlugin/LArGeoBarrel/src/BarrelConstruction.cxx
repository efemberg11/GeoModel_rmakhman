...
void LArGeo::BarrelConstruction::MakeEnvelope()
{
  ...
  */
  ...
  GeoInpRecordset_ptr barrelSaggingRec         = inpman->getRecordsetPtr("BarrelSagging");
  ...
  GeoInpRecordset_ptr barrelEtaTransRec        = inpman->getRecordsetPtr("BarrelEtaTrans");
  GeoInpRecordset_ptr coldContractionRec       = inpman->getRecordsetPtr("ColdContraction");

  GeoInpRecordset_ptr switchSet = inpman->getRecordsetPtr("LArSwitches");
  if ((*switchSet).size() !=0) {
...
//      log << MSG::DEBUG  << " no DETAILED_ABSORBER structure in DB " << endmsg;
    ...
    std::cout << "WARNING: LArSwitches structure not found " << std::endl;
  }

  std::cout << "  Makes detailed absorber sandwich  ? " << doDetailedAbsorberStraight << " " << doDetailedAbsorberFold << std::endl;
  std::cout << "  Use sagging in geometry  ? " << m_A_SAGGING << std::endl;
  ...
  double twopi64 = SYSTEM_OF_UNITS::pi/32.;
  ...
  double Moth_outer_radius = (*barrelGeometryRec)[0].getDouble("RMAX")*SYSTEM_OF_UNITS::cm;
  
  double Moth_Phi_Min = 0.;
  double Moth_Phi_Max = (*barrelGeometryRec)[0].getDouble("PHIMAXBARREL")*SYSTEM_OF_UNITS::deg;
  ...
  std::cout << "  phi1,Dphi (SYSTEM_OF_UNITS::deg)" << Moth_Phi_Min/SYSTEM_OF_UNITS::deg << " " << Moth_Phi_Max/SYSTEM_OF_UNITS::deg << std::endl;
#endif
  
  // number of zigs for accordion
  int    Nbrt = (int) ( (*barrelGeometryRec)[0].getInt("NBRT") );  // =14
  ...
  double Rhocen1 = (*barrelGeometryRec)[0].getDouble("RHOCEN_0")*SYSTEM_OF_UNITS::cm;
  
  // R and Z coordinates of ETA_cuts.
  double Bar_Eta_cut = (*barrelGeometryRec)[0].getDouble("ETACUT");
#ifdef DEBUGGEO
  ...
  // z max at low radius
  Bar_Z_cut = (*barrelGeometryRec)[0].getDouble("ZMIN")*SYSTEM_OF_UNITS::cm + (*barrelGeometryRec)[0].getDouble("G10FRONTDELTAZ")*SYSTEM_OF_UNITS::cm;
  // minimal radius at Bar_z_max
  Bar_Rcmx  = (*barrelGeometryRec)[0].getDouble("RMINHIGHZ")*SYSTEM_OF_UNITS::cm;
  ...
  for (int idat=0; idat<15; idat++) {
    ...
    if(idat == 14) Delta[idat]  = (90.0) * SYSTEM_OF_UNITS::deg; 

    // Maximum SAGGING displacement for each of the fifteen folds in SYSTEM_OF_UNITS::mm
	...
	<< Rhocen[idat] << " " << Phicen[idat]*(1./SYSTEM_OF_UNITS::deg) << " "
	<< Delta[idat]*(1./SYSTEM_OF_UNITS::deg) << " " << deltay[idat] << " " << deltax[idat]
	<< " " << etaTrans << std::endl;
    */
  }
  ...
  if (m_NVISLIM > 0) {
    std::cout << "BarrelConstruction WARNING" << std::endl;
    std::cout << "================LAr BarrelConstruction===========================" << std::endl;
    std::cout << "===YOU ARE RUNNING WITH A LIMITED SLICE OF BARREL ACCORDEON   ===" << std::endl;
    std::cout << "===THIS OPTION IS FOR VISUALIZATION OR OTHER TESTING, ONLY!!  ===" << std::endl;
    std::cout << "===AND IF THIS IS NOT YOUR INTENTION PLEASE BE SURE TO SET    ===" << std::endl;
    std::cout << "===THE FLAG GeoModelSvc.LArDetectorTool.BarrelCellVisLimit=-1 ===" << std::endl;
    std::cout << "===Remember there are no defaults in Athena. Thank you.       ===" << std::endl;
    std::cout << "=================================================================" << std::endl;
  }
...
// max z of Stac (@ r = Rhocen[0] )
  ...
  //-----------------ECAM---------------------------------------------------------//
  {                                                                               //
    double Moth_Phi_Min2 = (Ncell == 64) ? -1.555*SYSTEM_OF_UNITS::deg : 0.;                       //
    double Moth_Phi_Max2 = (Ncell == 64) ? 25.61*SYSTEM_OF_UNITS::deg  : 2*M_PI;                   //
                                                                                  //
    double safety_rhocen1 = 0.040*SYSTEM_OF_UNITS::mm;                                             //
    ...
    double Roacc[] = {Moth_outer_radius,Moth_outer_radius,Moth_outer_radius};     //
     ...
     std::cout << " PhiMin,Dphi " << Moth_Phi_Min2/SYSTEM_OF_UNITS::deg << " " << Moth_Phi_Max2/SYSTEM_OF_UNITS::deg << std::endl;
#endif
  ...
  double Xel1f;
  {
    // WARNING : this "hard_coded" 0.010*SYSTEM_OF_UNITS::mm is a "security" to avoid
    //           fake "overlapping" diagnostics with "DAVID"
    Xel1f = (*barrelGeometryRec)[0].getDouble("XEL1")*SYSTEM_OF_UNITS::cm;       // 23.*SYSTEM_OF_UNITS::mm
    double DeltaZ = Zhalfc;
    double Zpos = Zhalfc+Bar_Z_min; 
    double Rmini =  Moth_inner_radius + 0.010*SYSTEM_OF_UNITS::mm;
    double Rmaxi = Rmini+Xel1f - 0.010*SYSTEM_OF_UNITS::mm;
    std::string name = baseName + "TELF";
#ifdef DEBUGGEO
    std::cout << " *** parameters for TELF tubs " << std::endl;
    std::cout << " DeltaZ      " << DeltaZ << std::endl;
    std::cout << " Rmin/Rmax   " << Rmini << " " << Rmaxi << std::endl,
    std::cout << " PhiMin,Dphi " << Moth_Phi_Min/SYSTEM_OF_UNITS::deg << " " << Moth_Phi_Max/SYSTEM_OF_UNITS::deg << std::endl;
  ...
  {
    double ThickSum = 10.*SYSTEM_OF_UNITS::mm;    // FIXME should be in geometry database
    double Rmini = Moth_inner_radius+Xel1f-ThickSum;
    double Rmaxi = Moth_inner_radius+Xel1f -0.020*SYSTEM_OF_UNITS::mm;    // safety margin
    double DeltaZ = Zhalfc;
    double Zpos=0.;
  ...
  {
    double ClearancePS = (*barrelMotherboardsRec)[0].getDouble("CLEARANCE")*SYSTEM_OF_UNITS::cm;
    double RhoPosB = Moth_inner_radius + ClearancePS;
    double bdx = .5*((*barrelMotherboardsRec)[0].getDouble("THIMB")*SYSTEM_OF_UNITS::cm); // 4.3/2.*SYSTEM_OF_UNITS::mm
    double bdy = .5*((*barrelMotherboardsRec)[0].getDouble("HEIGHTMB")*SYSTEM_OF_UNITS::cm);     // 72.3/2.*SYSTEM_OF_UNITS::mm;
    double bdz = Zhalfc - 0.007*SYSTEM_OF_UNITS::mm;
    
    //------------------------MOTHERBOARDS--------------------------------------------//
    // JFB Make & Place the motherboards inside overall tube                          //
    {                                                                                 //
      double PhiOrig = 0.;                                                            //
      int NoOFboard = (int) (*barrelMotherboardsRec)[0].getInt("MOTHBOAR");            // 32
      ...
      std::cout << " Phi0,Dphi  " << PhiPos0/SYSTEM_OF_UNITS::deg << " " << twopi32/SYSTEM_OF_UNITS::deg << std::endl;
#endif
    ...
    {                                                                                 //
      //                                                                              //
      double Dzc = Zhalfc - 0.007*SYSTEM_OF_UNITS::mm;                                                 //
      double Dx1 = .5*((*barrelAccordionCablesRec)[0].getDouble("THICKIN")*SYSTEM_OF_UNITS::cm);                // 1./2.*SYSTEM_OF_UNITS::mm
      double Dx2 = .5*Bar_Eta_cut*((*barrelAccordionCablesRec)[0].getDouble("THICKFAC")*SYSTEM_OF_UNITS::cm);     //
      // Dx2 should be  5.17/2.*Bar_Eta_cut*SYSTEM_OF_UNITS::mm Trapezoid's side linear with Eta       //
      double Dy1 = .5*((*barrelAccordionCablesRec)[0].getDouble("HEIGHT")*SYSTEM_OF_UNITS::cm);               // 70./2.*SYSTEM_OF_UNITS::mm
      double Dy2 = Dy1;                                                               //
      //                                                                              //
      int NoOFcable = (int) (*barrelAccordionCablesRec)[0].getInt("ACCORCAB");        // 64
      ...
      GeoGenfun::GENFUNCTION PhiOrig  = 22.5*SYSTEM_OF_UNITS::deg*Int(Ccopy/4);                           //
	...
	GeoXF::Pow(GeoTrf::RotateZ3D(1.0),PhiPos);                                         //
      ...
      for (int ii=0;ii<NoOFcable;ii++) {
       std::cout << "copy, phi " << ii << " " << PhiPos(ii)/SYSTEM_OF_UNITS::deg << std::endl;
      }
#endif
      Elnicsf_phys->add(iD);                                                          //
      Elnicsf_phys->add(st);                                                          //
    ...
    // add 1.3 SYSTEM_OF_UNITS::mm in z to allow cleareance for absorber with non                    //
    // 0 thickness, at eta=1.475, low r part of the barrel                          //
    // this affects STAC and TELB volumes                                           //
    double clearance = 1.3*SYSTEM_OF_UNITS::mm;     
  ...
  {                                                                                 //
    ...
    double safety = 0.05*SYSTEM_OF_UNITS::mm;
    double DeltaZ = 0.5*(ze2-ze1)-safety;                           // 50 micron for safety.
    double Zpos = ze1+DeltaZ+0.5*safety;                                            // 
    double Rmini1 = Rhocen[0] - .030*SYSTEM_OF_UNITS::mm;                                            //
    double Rmaxi1 = Rhocen[0] - .020*SYSTEM_OF_UNITS::mm;                                            //
    double Rmini2 = Rhocen[0] - .030*SYSTEM_OF_UNITS::mm;                                            //
    double Rmaxi2 = Bar_Rcmx - clearance - .070*SYSTEM_OF_UNITS::mm;                                 //
    ...
    std::cout << " DeltaZ " << DeltaZ << std::endl;
    std::cout << " Phi_Min,Dphi " << Moth_Phi_Min/SYSTEM_OF_UNITS::deg << " " << Moth_Phi_Max/SYSTEM_OF_UNITS::deg << std::endl;
...
//
  ...
  //---------------------------------FRONT G10-------------------------------------//
  {                                                                                //
    double Xel1f = (*barrelGeometryRec)[0].getDouble("XEL1")*SYSTEM_OF_UNITS::cm;            // 23.*SYSTEM_OF_UNITS::mm
    double Xg10f = (*barrelGeometryRec)[0].getDouble("XG10")*SYSTEM_OF_UNITS::cm;            // 20.*SYSTEM_OF_UNITS::mm
    double DeltaZ = 0.5*(*barrelGeometryRec)[0].getDouble("G10FRONTDELTAZ")*SYSTEM_OF_UNITS::cm;           //
    ...
    GeoInpRecordset_ptr extraCones = inpman->getRecordsetPtr("LArCones");
  ...
  {                                                                               //
    ...
    double Xtal  = (*barrelGeometryRec)[0].getDouble("XTAL")*SYSTEM_OF_UNITS::cm + 0.1*SYSTEM_OF_UNITS::mm;            // 13.*SYSTEM_OF_UNITS::mm
    double Rmini = Rhocen[Nbrt]+Xtal; //                                          //
    // GU to be sure that GTENB does not go outside mother ECAM volume            //
    //  Rmaxi = Rmini+Xg10b;                                                      //
    double   Rmaxi = Moth_outer_radius-0.01*SYSTEM_OF_UNITS::mm;   // 10 microns for more safety.. //
    //                                                                            //
  ...
  {                                                                             //
    ...
    double Moth_Phi_Min2 = (Ncell == 64) ? -1.055*SYSTEM_OF_UNITS::deg : 0.;                     //
    double Moth_Phi_Max2 = (Ncell == 64) ? 24.61*SYSTEM_OF_UNITS::deg  : 2*M_PI;                 //
...
#ifdef BUILD_ACCORDION_PLATES
  {
    ...
    double Xtip_pb    = (*barrelGeometryRec)[0].getDouble("XTIP_PB")*SYSTEM_OF_UNITS::cm;
    double Xtip_pc    = (*barrelGeometryRec)[0].getDouble("XTIP_PC")*SYSTEM_OF_UNITS::cm;
    double Xtip_gt    = (*barrelGeometryRec)[0].getDouble("XTIP_GT")*SYSTEM_OF_UNITS::cm;
    double Xtip_gs    = (*barrelGeometryRec)[0].getDouble("XTIP_GS")*SYSTEM_OF_UNITS::cm;
    ...
    double Thpb_thin  = (*barrelGeometryRec)[0].getDouble("TGPB")*SYSTEM_OF_UNITS::cm;
    ...
    double Psi        = (*barrelGeometryRec)[0].getDouble("ALFA")*SYSTEM_OF_UNITS::deg;   // 360.*SYSTEM_OF_UNITS::deg/1024
    double Contract   = (*coldContractionRec)[0].getDouble("ABSORBERCONTRACTION");
...
// different parity depending on direction of first wave
	...
	// Absorber (thick, thin)
	{
	  ...
	  double Xhalfb  = fb==FRONT ? Xtip_pb/2 -0.002*SYSTEM_OF_UNITS::mm    : Xtipt/2 - .004*SYSTEM_OF_UNITS::mm;
	  ...
	  m_ecamPhysicalNeg->add(st);
          ...
          std::cout << " Phi0 (SYSTEM_OF_UNITS::deg) " << Gama(0)/SYSTEM_OF_UNITS::deg << std::endl;
	...
	if (fb==FRONT)
	{
	  double Xhalfbg = Xtip_gt/2-0.002*SYSTEM_OF_UNITS::mm;
...
// add small iron pieces on each side of the G10
          ...
          double Tgfe = (*barrelGeometryRec)[0].getDouble("TGFE")*SYSTEM_OF_UNITS::cm;
#ifdef DEBUGGEO
...
// position of G10+Steel inside LAr bath
	...
	// Electrode
	{
	  double Xhalfbe = fb==FRONT ? Xtips/2 -0.002*SYSTEM_OF_UNITS::mm      : Xtipt/2 - .004*SYSTEM_OF_UNITS::mm;
	  ...
	  m_ecamPhysicalNeg->add(st);
          ...
          std::cout << " Phi0 (SYSTEM_OF_UNITS::deg)" << Game(0)/SYSTEM_OF_UNITS::deg << std::endl;
...
// radius of the end and the beginning of the straight sections
	    ...
	    // translation in x to include thick absorber into thin absorber
	      double Xtrans = (Xb1+Xt1)/2.-Zx0 + .007*SYSTEM_OF_UNITS::mm;    
...
// last fold if up
		   ...
		   std::cout << " LArGeoBarrel::BarrelConstruction  fold not defined..." << std::endl;
                 }
                 else
                 for (int instance = 0; instance < Nabsorber; instance++)
                 {
