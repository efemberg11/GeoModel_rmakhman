...
GeoFullPhysVol* LArGeo::BarrelCryostatConstruction::GetEnvelope()
{
  ...
  if (larPosition->size()==0 ) throw std::runtime_error("Error, no lar position table in the input XML!");

  for (int n=0;n<3;n++) {

    const GeoInpRecord *posRec = LArGeoInpUtils::getTransformRecord(larPosition, names[n]);
  ...
  GeoInpRecordset_ptr cryoEars = inpman->getRecordsetPtr("CryoEars");
  ...
  // and adjust the volume geometry
  // accordingly.
  ...
  for(unsigned int ind=0; ind<cryoMotherPlanes.size(); ind++)
  {
    ...
    {
      const GeoInpRecord& currentRecord_inp = (*cryoPcons)[(*iter).second];

      cryoMotherShape->addPlane(currentRecord_inp.getDouble("ZPLANE"),
				currentRecord_inp.getDouble("RMIN"),
				currentRecord_inp.getDouble("RMAX"));
    }
  }
  ...
  {
    const GeoInpRecord& currentRecord_inp = (*cryoCylinders)[ind];

    if(currentRecord_inp.getString("CYL_LOCATION")=="Barrel::CryoMother")
    {

      const GeoMaterial *material  = materialManager->getMaterial(currentRecord_inp.getString("MATERIAL"));

      if (!material)
      {
	std::ostringstream errorMessage;
	errorMessage << "Error in BarrelCrysostat Construction" << std::endl;
	errorMessage << "Material " << currentRecord_inp.getString("MATERIAL") << " is not found" << std::endl;
	throw std::runtime_error(errorMessage.str().c_str());
      }


      std::ostringstream cylStream;
      int cylID = currentRecord_inp.getInt("CYL_ID");
      cylStream << "LAr::Barrel::Cryostat::Cylinder::#" << cylID;
      std::string cylName= cylStream.str();

      int cylNumber = currentRecord_inp.getInt("CYL_NUMBER");
      double zMin = currentRecord_inp.getDouble("ZMIN")*SYSTEM_OF_UNITS::cm;
      double dZ   = currentRecord_inp.getDouble("DZ")*SYSTEM_OF_UNITS::cm;
      double zInCryostat = zMin + dZ / 2.;

      if(m_fullGeo){
	if ((*cryoEars).size()>0 && cylID==1) {
	  const GeoInpRecord& record = (*cryoEars)[0];
	...
	solidBarrelCylinder
	  = new GeoTubs(currentRecord_inp.getDouble("RMIN")*SYSTEM_OF_UNITS::cm,
			currentRecord_inp.getDouble("RMIN")*SYSTEM_OF_UNITS::cm + currentRecord_inp.getDouble("DR")*SYSTEM_OF_UNITS::cm,
			currentRecord_inp.getDouble("DZ")*SYSTEM_OF_UNITS::cm / 2.,
			(double) 0.,
			dphi_all);
      if(m_activateFT){
	  ...
	  /*
          log << MSG::DEBUG << "Cut holes for feedthroughs in warm wall "
              ...
              << std::endl;
	  */
          const double rmin = currentRecord_inp.getDouble("RMIN")*SYSTEM_OF_UNITS::cm;
          const double rmax = currentRecord_inp.getDouble("RMIN")*SYSTEM_OF_UNITS::cm + currentRecord_inp.getDouble("DR")*SYSTEM_OF_UNITS::cm;
          const double bellow_Router = 0.5*299.*SYSTEM_OF_UNITS::mm; // from DMconstruction
          const double warmhole_radius = bellow_Router;
          const double warmhole_pos = 4.*SYSTEM_OF_UNITS::cm;
          ...
          for(int i = 0; i < NCrates; ++ i){
            const double phi = 360.*SYSTEM_OF_UNITS::deg * i / NCrates;
            put(phi - dphi);
            put(phi + dphi);
          }
	  ...
	  */
          const double rmin = currentRecord_inp.getDouble("RMIN")*SYSTEM_OF_UNITS::cm;
          const double rmax = currentRecord_inp.getDouble("RMIN")*SYSTEM_OF_UNITS::cm + currentRecord_inp.getDouble("DR")*SYSTEM_OF_UNITS::cm;
          const double coldhole_radius = 0.5*150.*SYSTEM_OF_UNITS::mm; // see DMconstruction
          const double coldhole_pos = 27.5*SYSTEM_OF_UNITS::mm;
	...
	if(zMin<0) {
	  ...
	  */
      ...
      if(m_activateFT && (cylID == 2 || cylID == 7)){
          m_cryoMotherPhysical->add(new GeoTransform(GeoTrf::RotateY3D(180.*SYSTEM_OF_UNITS::deg)));
          m_cryoMotherPhysical->add(new GeoTransform(GeoTrf::TranslateZ3D(zInCryostat)));
      } else {
          m_cryoMotherPhysical->add(new GeoTransform(GeoTrf::TranslateZ3D(-zInCryostat)));
      }
  ...
  if(m_fullGeo) {
	...
	const GeoInpRecord& currentRecord_inp = (*cryoPcons)[(*iter).second];
	outerWallPcon->addPlane(currentRecord_inp.getDouble("ZPLANE"),
				currentRecord_inp.getDouble("RMIN"),
				currentRecord_inp.getDouble("RMAX"));
      }
    }
	...
	const GeoInpRecord& currentRecord_inp = (*cryoPcons)[(*iter).second];
	innerWallPcon->addPlane(currentRecord_inp.getDouble("ZPLANE"),
				currentRecord_inp.getDouble("RMIN"),
				currentRecord_inp.getDouble("RMAX"));
      }
    }
    ...
    GeoInpRecordset_ptr cryoBolts = inpman->getRecordsetPtr("LArBarrelCryoBolts");

    if (cryoBolts->size() >0) {
      std::cout << " new description with barrel croystat bolts" << std::endl;
        const GeoInpRecord& cryoBoltsRecord = (*cryoBolts) [0];
        ...
        if ((iter = innerWallPlanes.find(index1))!= innerWallPlanes.end()) {
          z1           =  (*cryoPcons)[(*iter).second].getDouble("ZPLANE");
        }
        if ((iter = innerWallPlanes.find(index2))!= innerWallPlanes.end()) {
          z2           =  (*cryoPcons)[(*iter).second].getDouble("ZPLANE");
        }
        if (z1>0. && z2>0.) {
       ...
       std::cout << " old description withut bold in the geometry database " << std::endl;
     }
    ...
    // Throw in coil support block
    {
      ...
      GeoInpRecordset_ptr newBlocks = inpman->getRecordsetPtr("LArBarBumperBlocks");

      if (newBlocks->size() >0 ) {
	std::cout << " new coil bumper description " << std::endl;
            const GeoInpRecord& newBlocksRecord = (*newBlocks) [0];
	...
	std::cout << " old coil bumper description " << std::endl;
	GeoInpRecordset_ptr tiBlocks = inpman->getRecordsetPtr("TiBlocks");

        const GeoInpRecord& tiBlocksRecord = (*tiBlocks)  [0];
      ...
      if ((iter = innerWallPlanes.find(indexWall))!= innerWallPlanes.end()) {
        const GeoInpRecord& pconsRecord = (*cryoPcons)  [(*iter).second];
...
// position of bumper in old description
       ...
       std::cout << "BarrelCryostatConstruction WARNING: could not find wall index plane => no coil bumper description " << std::endl;
     }
	...
	if(iter==innerEndWallPlanes.end())
	  ...
	  const GeoInpRecord& currentRecord_inp = (*cryoPcons)[(*iter).second];
	  innerEndWallPcon->addPlane(currentRecord_inp.getDouble("ZPLANE"),
				     currentRecord_inp.getDouble("RMIN"),
				     currentRecord_inp.getDouble("RMAX"));
	}
      }
  ...
  for(unsigned int ind=0; ind<totalLarPlanes.size(); ind++)
  {
    ...
    {
      const GeoInpRecord& currentRecord_inp = (*cryoPcons)[(*iter).second];
      totalLArShape->addPlane(currentRecord_inp.getDouble("ZPLANE"),
			      currentRecord_inp.getDouble("RMIN"),
			      currentRecord_inp.getDouble("RMAX"));
    }
  }
  ...
  for(unsigned int ind=0; ind<halfLarPlanes.size(); ind++)
  {
    ...
    {
      const GeoInpRecord& currentRecord_inp = (*cryoPcons)[(*iter).second];
      halfLArShape->addPlane(currentRecord_inp.getDouble("ZPLANE"),
			     currentRecord_inp.getDouble("RMIN"),
			     currentRecord_inp.getDouble("RMAX"));
    }
  }
  ...
  totalLArPhysical->add( new GeoTransform(GeoTrf::RotateY3D(180.*SYSTEM_OF_UNITS::deg)) );
...

  {
    ...
    if(m_fullGeo) {
      ...
      //
      for (unsigned int ind=0; ind < cryoCylinders->size(); ind++) {
	const GeoInpRecord& currentRecord = (*cryoCylinders)[ind];
	...
	if(currentRecord.getString("CYL_LOCATION")=="Barrel::HalfLAr"||
	  ...
	  = new GeoTubs(currentRecord.getDouble("RMIN")*SYSTEM_OF_UNITS::cm,
	  ...
	  double zInCryostat = currentRecord.getDouble("ZMIN")*SYSTEM_OF_UNITS::cm + currentRecord.getDouble("DZ")*SYSTEM_OF_UNITS::cm / 2.;
  ...
  {
    // ----- Presampler ------
    double PresamplerMother_length = 1549.0*SYSTEM_OF_UNITS::mm;  // Copied from PresParameterDef.icc
    double presamplerShift = 3.*SYSTEM_OF_UNITS::mm;
  ...
  // SCT-EC Cooling
  GeoInpRecordset_ptr cryoPconPhiSect = inpman->getRecordsetPtr("CryoPconPhiSect");
  ...
  for(unsigned i=0; i<cryoPconPhiSect->size(); ++i) {
    ...
    GeoPcon* pcon = new GeoPcon(startPhi*SYSTEM_OF_UNITS::deg,dPhi*SYSTEM_OF_UNITS::deg);
	...
	const GeoInpRecord& currentRecord_inp = (*cryoPcons)[(*iter).second];
	pcon->addPlane(currentRecord_inp.getDouble("ZPLANE"),
		       currentRecord_inp.getDouble("RMIN"),
		       currentRecord_inp.getDouble("RMAX"));
      }
    } // iterate over planes
    ...
    GeoTransform* xfPos2 = new GeoTransform(GeoTrf::RotateZ3D(180*SYSTEM_OF_UNITS::deg));
    GeoTransform* xfNeg1 = new GeoTransform(GeoTrf::RotateZ3D((180+2*centerPhi)*SYSTEM_OF_UNITS::deg)*GeoTrf::RotateY3D(180*SYSTEM_OF_UNITS::deg));
    GeoTransform* xfNeg2 = new GeoTransform(GeoTrf::RotateZ3D(2*centerPhi*SYSTEM_OF_UNITS::deg)*GeoTrf::RotateY3D(180*SYSTEM_OF_UNITS::deg));
