  ...
  , m_fullGeo(fullGeo)
{
  std::cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
  std::cout << "+                                                  +" << std::endl;
  std::cout << "+         Start of Barrel PS GeoModel definition   +" << std::endl;
  std::cout << "+                                                  +" << std::endl;
  std::cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
  ...
  //  double rMinPresamplerMother   =1385*SYSTEM_OF_UNITS::mm;
  double rMinPresamplerMother   =1410*SYSTEM_OF_UNITS::mm;
  double rMaxPresamplerMother   =1447*SYSTEM_OF_UNITS::mm-0.001*SYSTEM_OF_UNITS::mm;
  double presamplerMother_length=1549*SYSTEM_OF_UNITS::mm;
  double Phi_min=0.*SYSTEM_OF_UNITS::deg;
  double Phi_span=360.*SYSTEM_OF_UNITS::deg;
  int nbsectors=32;

  if (itb==1) {
     Phi_min=-0.5*SYSTEM_OF_UNITS::deg;
     Phi_span=23.5*SYSTEM_OF_UNITS::deg;
     nbsectors=2;
  }
  ...
  if(m_fullGeo){
    // ?    
    double  epsil = 0.007*SYSTEM_OF_UNITS::mm;

    //  contraction factor 
    double  cmm = (1-0.0026)*SYSTEM_OF_UNITS::mm;
 
    double  mod_leng[8];
    for(int ii=0; ii<8; ii++ ) mod_leng[ii]=mod[ii][0]*cmm+2*epsil;

    double  mod_heig[8];
    double larheight = 13*SYSTEM_OF_UNITS::mm;

    double prep1_th = 1.*SYSTEM_OF_UNITS::mm;                 // bottom prepreg layer 
    double prep2_th = 4.5*SYSTEM_OF_UNITS::mm;   
    double smallLength = 275.6*SYSTEM_OF_UNITS::mm;
    double bigLength = 277.5;
    double prep1_height = (smallLength/2+1.)*cmm;
    double larheight2 = larheight*cos(-mod[1][3]*SYSTEM_OF_UNITS::deg)*SYSTEM_OF_UNITS::mm;
    ...
    double shell_th = 0.4*SYSTEM_OF_UNITS::mm; 
    double rail_th  = 8.6*SYSTEM_OF_UNITS::mm;
    double mech_clear = 0.5*SYSTEM_OF_UNITS::mm;


    double  mb_length = 3100.3;
    double  sector_length =  mb_length*cmm +9.*epsil;
    double  sector_height =  mod_heig[0]+(shell_th+rail_th)*cmm+mech_clear*SYSTEM_OF_UNITS::mm+3*epsil; 

    unsigned int nsectors=32;
    double mod_xm = prep1_height+epsil;
    double mod_xp = (bigLength/2+1.+prep2_th*tan((360./(2*nsectors))*SYSTEM_OF_UNITS::deg))*cmm;
    double sect_xm = mod_xm+epsil;
    double sect_xp = sect_xm+sector_height*tan((360./(2*nsectors))*SYSTEM_OF_UNITS::deg);	    
    double rpres = 1426.*SYSTEM_OF_UNITS::mm;
    ...
    double dphiSector = (360.*SYSTEM_OF_UNITS::deg)/nsectors;
    GeoGenfun::GENFUNCTION  f = dphiSector*I+0.5*dphiSector;
    GeoXF::TRANSFUNCTION t = GeoXF::Pow(GeoTrf::RotateZ3D(1.0),f)*GeoTrf::TranslateX3D(rpres)*GeoTrf::TranslateZ3D(zpres)*GeoTrf::RotateZ3D(90*SYSTEM_OF_UNITS::deg)*GeoTrf::RotateX3D(90*SYSTEM_OF_UNITS::deg);
    ...
    double delta01 =  0.5*larheight*tan(-mod[1][3]*SYSTEM_OF_UNITS::deg);   // delta01 is >0
...
//  	double modLen=mod[m][0]*cmm+2*epsil;
    ...
    double glX = 0.*SYSTEM_OF_UNITS::mm;
    ...
    //-----------------------------Connectics--------------------------//
    {
      ...
      GeoTransform* xf2 = new GeoTransform(GeoTrf::RotateX3D(-90*SYSTEM_OF_UNITS::deg));
    ...
    double heig_elec1 = (larheight/cos(-mod[0][3]*SYSTEM_OF_UNITS::deg)-0.5*anode_th/cos(mod[0][3]*SYSTEM_OF_UNITS::deg))*cmm;
    double heig_elec3 = (larheight-0.5*cathode_th/cos(mod[1][3]*SYSTEM_OF_UNITS::deg))*cmm;
...
// within full sector
    ...
    for(int i=0; i<8; i++)
    {
      ...
      GeoXF::TRANSFUNCTION cathoTF = GeoXF::Pow(GeoTrf::TranslateY3D(1.),cathoGF)*GeoTrf::TranslateZ3D(elec_trans)*GeoTrf::RotateX3D(-mod[i][3]*SYSTEM_OF_UNITS::deg);
      GeoXF::TRANSFUNCTION anoTF = GeoXF::Pow(GeoTrf::TranslateY3D(1.),anoGF)*GeoTrf::TranslateZ3D(elec_trans)*GeoTrf::RotateX3D(-mod[i][3]*SYSTEM_OF_UNITS::deg);
