...
// std::cout<<"===================> FRAME in sTGC.cxx ====> f4= "<<f4<<" f5= "<<f5<<" f6= "<<f6<<std::endl;
  ...
  const GeoMaterial* mtrd = MaterialManager::getMaterial("muo::Honeycomb");
  ...
  for (int i = 0; i < t->nlayers; i++) {
    ...
    GeoLogVol* ltrdgas = new GeoLogVol("sTGC_Sensitive", sGasVolume1, MaterialManager::getMaterial("muo::TGCGas"));
    ...
    for(int i = 0; i < 2; i++){
      ...
      const GeoMaterial* mtrdC = MaterialManager::getMaterial("std::G10");
